...
- **Voice Testing**: `test.py` - Daily room creation for voice testing with STT/TTS
- **Text Testing**: `chat_test.py` - Fast text-only chat interface (no STT/TTS for rapid testing)
- **Bridge Service**: `talkdeskbridge/bridge_conn.py` - Handles Talkdesk audio forwarding
...
2. **Voice Testing** - `test.py`
   ```bash
   python test.py                              # Full flow (greeting) - WITH VOICE
...
3. **Recommended Workflow**
   ...
   - ✅ Use `test.py` for final voice validation (10% of development)
   - ✅ All flows work identically in both modes

### Production Deployment Process
1. **Code Changes**: Make changes and test locally with `test.py`
...
├── test.py                        # Voice testing with Daily rooms (STT/TTS)
├── chat_test.py                   # Text-only testing (no STT/TTS) - FAST!
├── README_CHAT_TESTING.md         # Text chat testing documentation
├── talkdeskbridge/bridge_conn.py  # Talkdesk audio bridge (Azure hosted)
...
├── pipeline/components.py         # Pipecat service creation
...
# Test specific flow nodes (works with both test.py and chat_test.py)
python test.py --start-node greeting        # Full flow from start (default)
python test.py --start-node email           # Email collection with STT switching
python test.py --start-node name            # Name collection
python test.py --start-node phone           # Phone number collection
python test.py --start-node fiscal_code     # Italian tax code collection
python test.py --start-node booking         # Pre-filled service/center, start at date selection
python test.py --start-node slot_selection  # Advanced testing with full state
python test.py --start-node cerba_card      # Cerba membership question

# Simulate existing patient (Talkdesk caller ID)
python test.py --caller-phone +393491234567 --patient-dob 1979-06-19
...
```bash
DAILY_API_KEY=your_key_here             # For test.py Daily room creation
...
4. **Test Locally**: Use `test.py --start-node {purpose}`
...
- **Usage**: Always test new flows with `test.py` before deployment
...
### Debugging Tools
- **Local Testing**: `python test.py --debug` for detailed logging
